// S4b-1 loop-gate negative control — week 포털 A(마운트 루프)/B(클릭 루프).
// 테스트 전용 — __S4B_LOOP_NEG_CTRL_A__/__S4B_LOOP_NEG_CTRL_B__===false 시 dead-branch tree-shake.
//
...
//    mount 시 GREEN, generate-week-note 클릭 후 re-render 시 #185 루프 → RED.
...
export function NegativeControlWeekIslandPortalA(): React.ReactElement | null {
...
//
// 설계: mount 시 안전(loop 없음). generate-week-note 클릭 후 첫 weekProps
...
export function NegativeControlWeekIslandPortalB(): React.ReactElement | null {
  // click 위임 등록 — generate-week-note 클릭 시 flag 활성화 + React notify
  React.useEffect(() => {
    function handleClick(e: MouseEvent): void {
      const target = e.target;
      if (target instanceof Element && target.closest("[data-action='generate-week-note']")) {
        if (!_bFlag) {
          _bFlag = true;
