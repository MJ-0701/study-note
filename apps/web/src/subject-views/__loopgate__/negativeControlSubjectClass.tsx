// S4b-2 loop-gate negative control — subject-class 포털 A(마운트 루프)/B(클릭 루프).
// 테스트 전용 — __S4B2_LOOP_NEG_CTRL_A__/__S4B2_LOOP_NEG_CTRL_B__===false 시 dead-branch tree-shake.
//
...
//    mount 시 GREEN, add-class-date submit(또는 navigation) 후 re-render 시 #185 루프 → RED.
import React, { useSyncExternalStore } from "react";
import { createPortal } from "react-dom";
import { useStore } from "zustand";
import { uiStore } from "../../stores/uiStore.ts";
import { SubjectClassView } from "../SubjectClassView.tsx";
...
export function NegativeControlSubjectClassIslandPortalA(): React.ReactElement | null {
  ...
  return createPortal(<SubjectClassView {...props} />, slot);
}

// ─── Negative Control B (click-time → useSyncExternalStore 불안정 활성화, §5-C) ─
//
// 설계: mount 시 안전(loop 없음). 사용자가 add-class-date form submit(또는 navigation) 후
// subjectClassProps 변경(uiStore) → portal re-render 시 unstable snapshot 이 활성화 → #185 루프.
//
// 구현: 독립 "flag store" 를 구현. document 클릭/submit listener 가 flag 를 set 하고
...
export function NegativeControlSubjectClassIslandPortalB(): React.ReactElement | null {
  // click 위임 등록 — add-class-date submit 시 flag 활성화 + React notify
  React.useEffect(() => {
    function handleClick(e: MouseEvent): void {
      const target = e.target;
      if (target instanceof Element && (
        target.closest("form[data-action='add-class-date']") ||
        target.closest("[data-action='attach-pdf-to-week']")
      )) {
        if (!_bFlag) {
          _bFlag = true;
