// S4c loop-gate negative control — pdf-workspaces 포털 A(마운트 루프)/B(클릭 루프).
// 테스트 전용 — __S4C_LOOP_NEG_CTRL_A__/__S4C_LOOP_NEG_CTRL_B__===false 시 dead-branch tree-shake.
//
// A (mount-time): unstable-getSnapshot useSyncExternalStore → mount 시 #185.
// B (click-time, §5-C 맹점 close): open-pdf-material 버튼 클릭-triggered unstable snapshot.
//    mount 시 GREEN, 클릭 후 re-render 시 #185 루프 → RED.
import React, { useSyncExternalStore } from "react";
import { createPortal } from "react-dom";
import { useStore } from "zustand";
import { uiStore } from "../../stores/uiStore.ts";
import { PdfWorkspacesView } from "../PdfWorkspacesView.tsx";
...
export function NegativeControlPdfWorkspacesIslandPortalA(): React.ReactElement | null {
...
//
// 설계: mount 시 안전(loop 없음). open-pdf-material 버튼 클릭 후
// pdfWorkspacesProps 변경(uiStore) → portal re-render 시 unstable snapshot 이 활성화 → #185 루프.
//
// 구현: 독립 "flag store" 를 구현. document 클릭 listener 가 flag 를 set 하고
...
export function NegativeControlPdfWorkspacesIslandPortalB(): React.ReactElement | null {
  // open-pdf-material 버튼 클릭 위임 등록 — 클릭 시 flag 활성화 + React notify
  React.useEffect(() => {
    function handleClick(e: MouseEvent): void {
      const target = e.target;
      if (target instanceof Element && target.closest("[data-action='open-pdf-material']")) {
        if (!_bFlag) {
          _bFlag = true;
