...
const COMPUTER_INTRODUCTION: SubjectExamPrepArtifact = {
  ...
  title: "컴퓨터개론 2024년도 시험문제 풀이 템플릿",
  sourceLabel: "2024년도 시험문제 + 8, 9, 10, 13장 수업자료",
  markdownHref: "",
  note: "타이핑한 2024년도 문제 8문항을 시험 답안 형태로 정리하고, 필요한 선행개념을 문제 바로 앞에서 확인하도록 묶었습니다.",
  studyOrder: [
    "9장 데이터베이스의 키와 3단계 스키마를 먼저 단답식으로 고정",
    "MPEG GOP와 손실/비손실 압축은 I/P/B 프레임 역할과 예시를 표처럼 암기",
    "10장 네트워크는 회선교환/패킷교환, DNS 서버 종류를 비교형으로 정리",
    "13장 정보보안은 공개키/비공개키 암호와 스니핑/스푸핑을 한 문장 정의로 반복"
  ],
  chapters: [
    {
      label: "9장",
    ...
    },
    {
      label: "24년",
      title: "MPEG와 압축",
      focus: "GOP, I/P/B 프레임, 손실 압축과 비손실 압축의 차이와 예",
    ...
    },
    {
      label: "8장",
      title: "프로그래밍 언어1",
      focus: "시험범위 수업자료로 유지하되 2024년 타이핑 문제에는 직접 문항 없음",
  ...
  concepts: [
    ...
    {
      title: "데이터베이스 3단계 추상화",
      points: [
        "외부 단계는 사용자나 응용 프로그램별로 보이는 데이터 관점이다.",
        "개념 단계는 전체 데이터베이스의 논리적 구조다.",
        "내부 단계는 데이터가 실제 저장장치에 저장되는 물리적 구조다."
      ]
    },
    {
      title: "MPEG GOP와 압축",
      points: [
        "I 프레임은 독립적으로 복원 가능한 기준 프레임이다.",
        "P 프레임은 이전 I/P 프레임을 기준으로 예측한 차이를 저장한다.",
        "B 프레임은 이전과 이후 프레임을 모두 참조해 압축률을 높인다.",
        "손실 압축은 일부 정보를 버리고, 비손실 압축은 원본을 정확히 복원한다."
      ]
    },
    {
      title: "네트워크 서비스 비교",
      points: [
        ...
        "패킷교환은 패킷마다 적절한 경로를 찾아 전달하고 인터넷이 대표 예다.",
        "DNS는 사람이 읽는 도메인 이름을 IP 주소로 바꾸는 이름 해석 서비스다."
      ]
    },
    {
      title: "정보보안 기본 답안",
      points: [
        ...
        "비공개키 암호는 AES처럼 송수신자가 같은 비밀키를 공유한다.",
  ...
  questions: [
    ...
    },
    {
      ...
      tags: ["MPEG", "GOP"],
      answer: [
        "I 프레임은 다른 프레임을 참조하지 않고 자체 정보만으로 복원되는 기준 프레임이다.",
        "P 프레임은 이전 I 또는 P 프레임을 기준으로 움직임과 차이 정보를 저장하는 예측 프레임이다.",
        "B 프레임은 이전 프레임과 이후 프레임을 모두 참조해 양방향 예측으로 압축률을 높이는 프레임이다."
      ],
      explanation: [
        "I 프레임은 용량이 크지만 임의 접근과 복구 기준이 된다.",
        "P 프레임은 뒤의 P/B 프레임이 참조할 수 있고, B 프레임은 압축률이 가장 높지만 보통 기준 프레임으로는 쓰지 않는다."
      ]
    },
    {
      id: "ci-q-crypto",
    ...
    },
    {
      ...
      tags: ["압축", "멀티미디어"],
      answer: [
        "A: 손실 압축은 원본의 일부 정보를 버려 압축률을 높이므로 완전한 원본 복원이 불가능하고, 비손실 압축은 정보를 버리지 않아 원본을 정확히 복원할 수 있다.",
        "B: 손실 압축의 예는 JPEG, MP3, MPEG 중 하나를 쓰면 된다.",
        "C: 비손실 압축의 예는 ZIP, PNG, GIF, RLE, Huffman coding 중 하나를 쓰면 된다."
      ],
      explanation: [
        "손실 압축은 사람이 덜 민감하게 느끼는 정보를 제거해 사진, 음악, 동영상에 자주 사용한다.",
        "비손실 압축은 문서, 프로그램, 데이터 파일처럼 한 비트라도 달라지면 안 되는 자료에 적합하다."
      ]
    },
    {
      id: "ci-q-switching",
  ...
  terms: [
    ...
    { term: "GOP", definition: "MPEG에서 I/P/B 프레임을 묶어 압축 효율과 임의 접근을 조절하는 프레임 그룹", note: "Group of Picture" },
    { term: "I 프레임", definition: "다른 프레임을 참조하지 않는 독립 프레임", note: "기준 프레임" },
    { term: "P 프레임", definition: "이전 I/P 프레임을 참조하는 예측 프레임", note: "차이 정보 저장" },
    { term: "B 프레임", definition: "이전과 이후 프레임을 참조하는 양방향 예측 프레임", note: "압축률 높음" },
  ...
  checklist: [
    ...
    "I/P/B 프레임을 독립, 이전 예측, 양방향 예측 순서로 말하기",
    "RSA는 공개키, AES는 비공개키/대칭키로 고정",
    "손실 압축 예 JPEG/MP3/MPEG, 비손실 압축 예 ZIP/PNG",
    "회선교환은 전화망, 패킷교환은 인터넷",
    "외부/개념/내부 스키마 순서 암기",
    "DNS 서버 종류는 수업자료 용어와 대체 용어를 둘 다 대비",
    "스니핑은 도청, 스푸핑은 위장"
  ]
};
