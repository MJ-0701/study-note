...
import { IngestService } from "../corpus/services/ingest.service";
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    } else if (token === "--subject" && i + 1 < argv.length) {
      subject = argv[++i];
  ...
  if (!pdfPath) {
    throw new Error(
      "usage: node backend/dist/cli/ingest-pdf.js --path <pdf-path> [--subject <subject-slug>]"
    );
  }
