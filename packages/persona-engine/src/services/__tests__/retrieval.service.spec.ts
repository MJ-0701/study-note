...
describe("cosineRank", () => {
  it("returns top-k by descending cosine over L2-normalized vectors", () => {
    ...
    assert.ok(first);
    assert.ok(second);
    assert.equal(first.ord, 0);
    assert.equal(second.ord, 1);
    assert.ok(first.score > second.score, "rank 0 must outscore rank 1");
  });
  ...
  it("dot product equals explicit cosine on L2-normalized vectors (1e-6 tolerance)", () => {
    ...
    assert.ok(first);
    assert.ok(
      Math.abs(first.score - explicitCosine) < 1e-6,
      `dot product ${first.score} must equal explicit cosine ${explicitCosine}`
    );
  });
});
