...
> **라이브 대시보드 운영 상태**: 🔴 **비활성** (비용 관리를 위해 Grafana + Prometheus 중단 — Azure 최소비용 셋팅. 아래 스냅샷으로 동일 화면 확인. 지원·심사 시점 재활성화는 toggle 만으로 복원되며 secret/volume 은 보존됨, 절차 = `docs/runbooks/observability-toggle.md`)
>
